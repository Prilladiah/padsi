...
export default function LaporanPendapatanPage() {
  ...
  const totalPendapatan = pendapatanData.reduce((sum, item) => sum + item.subTotal, 0);
  ...
  const downloadPDF = () => {
    ...
    const tableHtml = `
      ...
      <html>
      <head>
        <title>Laporan Pendapatan - Sanguku</title>
        <style>
          body { 
      ...
      <body>
        <div class="header">
          <h1>LAPORAN PENDAPATAN SANGUKU</h1>
          <p>Sistem Informasi Pengelolaan Keuangan</p>
        </div>

        <div class="info">
          <strong>Total Data:</strong> ${pendapatanData.length} transaksi | 
        ...
        <div class="total-section">
          <h3>TOTAL PENDAPATAN: ${formatCurrency(totalPendapatan)}</h3>
          <p>Dari ${pendapatanData.length} transaksi pendapatan</p>
        </div>
  ...
  const downloadCSV = () => {
    const headers = ['Tanggal', 'Unit Bisnis', 'Stok', 'Jumlah', 'Metode Pembayaran', 'Sub Total Pendapatan'];
    
    const csvData = pendapatanData.map(item => [
      item.tanggal,
      item.unitBisnis,
      item.stok,
      item.jumlah,
    ...
    link.setAttribute('download', `laporan-pendapatan-sanguku-${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="text-left">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Laporan Pendapatan</h1>
          </div>
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="text-sm text-gray-600">
            Menampilkan {startIndex + 1}-{Math.min(startIndex + itemsPerPage, pendapatanData.length)} dari {pendapatanData.length} transaksi
          </div>
          <div className="flex gap-2">
            <button
        ...
        <div className="bg-white rounded-lg shadow border p-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 text-sm">Total keseluruhan pendapatan</p>
            </div>
            <div className="text-right">
              ...
              <div className="text-sm text-gray-500">
                dari {pendapatanData.length} transaksi
              </div>
            </div>
          </div>
        </div>
